...
def _frame() -> pd.DataFrame:
    rows = [
        ...
        dict(order_id="C1", customer_unique_id="U4", purchase_date="2018-02-15", order_status="delivered", customer_state="SP", customer_city="campinas", seller_state="MG", product_category="health", total_item_value=80.0, review_score=3.0, delivery_days=14.0, estimated_delivery_days=13.0, days_late=1.0, is_delivered_complete=True, is_late=True, is_negative_review=False),
        dict(order_id="D1", customer_unique_id="U5", purchase_date="2017-03-12", order_status="canceled", customer_state="SP", customer_city="sao paulo", seller_state="SP", product_category="books", total_item_value=90.0, review_score=None, delivery_days=None, estimated_delivery_days=10.0, days_late=None, is_delivered_complete=False, is_late=pd.NA, is_negative_review=pd.NA),
    ]
    ...
    frame["route_type"] = frame["customer_state"].eq(frame["seller_state"]).map({True: "Same state", False: "Cross state"})
    return frame
...
def test_every_filter_dimension_individually():
    ...
    assert set(_apply(frame, customer_cities=["campinas"])["order_id"]) == {"C1"}
...
def test_all_visuals_use_the_same_multi_filtered_population(monkeypatch):
    ...
    expected_keys = {
        "overview-indexed-growth",
        "overview-orders",
        "overview-aov",
        "overview-pareto",
        "promise-quoted-actual",
        "promise-timing-counts",
        "experience-timing",
        "experience-route-days",
        "experience-route-late",
        "experience-route-heatmap",
    }
    assert expected_keys.issubset(figures), expected_keys.difference(figures)

    # Business overview visual populations
    assert sum(figures["overview-orders"].data[0].y) == 2
    pareto_bar = figures["overview-pareto"].data[0]
    assert list(pareto_bar.x) == ["books"]
    assert sum(pareto_bar.y) == 2

    # Delivery promise visual populations
    ...
    # Customer experience visual populations
    ...
    for i in range(12):
        extra.append(
            dict(
                ...
                is_delivered_complete=True, is_late=False, is_negative_review=False, route_type="Same state",
            )
        )
